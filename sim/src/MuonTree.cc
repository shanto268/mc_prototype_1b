...
MuonTree::MuonTree(int dummy){
   ...
   _MuonXG4OutName="muonTree01.root";
   char* _param1;
   _param1 = getenv("MuonXG4OutName");
   if(_param1 != NULL) {string ss(_param1); _MuonXG4OutName=ss;}
...
//   histo1D["trig_LengthWall"]=new TH1D("trig_LengthWall","Track Length in Wall (trig)",100,0.,50.0);
   ...
   tree=new TTree("tree","Cosmic Muon Tree");
    ...
    tree->Branch("EdepTestObj"      ,&mEdepTestObj    , "EdepTestObj/F");
    tree->Branch("mStepTestObj"      ,&mStepTestObj    , "mStepTestObj/F");
...
//	tree->Branch("LengthWall"      ,&mLengthWall    , "LengthWall");
    ...
    tree->Branch("nHitsT1"        ,&mNHitsT1     , "nHitsT1/I" );
    tree->Branch("HitsT1Vx"       ,mHitsT1Vx     , "HitsT1Vx[nHitsT1]/F");
    tree->Branch("HitsT1Vy"       ,mHitsT1Vy     , "HitsT1Vy[nHitsT1]/F");
    tree->Branch("HitsT1Vz"       ,mHitsT1Vz     , "HitsT1Vz[nHitsT1]/F");

    tree->Branch("nHitsT2"        ,&mNHitsT2     , "nHitsT2/I" );
    tree->Branch("HitsT2Vx"       ,mHitsT2Vx     , "HitsT2Vx[nHitsT2]/F");
    tree->Branch("HitsT2Vy"       ,mHitsT2Vy     , "HitsT2Vy[nHitsT2]/F");
    tree->Branch("HitsT2Vz"       ,mHitsT2Vz     , "HitsT2Vz[nHitsT2]/F");
...
void MuonTree::analyze(SC8edep edepSc8, vector<SC8Particle> part,
     ...
     std::vector<SC8Particle> hitsT1,
     std::vector<SC8Particle> hitsT2,
...
//    histo1D["all_muon_step"]->Fill(mMuonLength[i]);  //changed by SAS 29/11
  ...
  mEdepTestObj=edepSc8.EdepTestObj;
  mStepTestObj=edepSc8.StepTestObj;
...
//SAS Comment: plotting all histo for triggered Stepl events
  ...
  mNHitsT1=hitsT1.size();
  //if(trig) histo1D["trig_Tray1Muonhits"]->Fill(mNHitsR1);
  for(int i=0; i<mNHitsT1; i++){
     mHitsT1Vx[i]=hitsT1[i].x;//hitsT1[i].x;
     mHitsT1Vy[i]=hitsT1[i].y;
     mHitsT1Vz[i]=hitsT1[i].z;
     }

   mNHitsT2=hitsT2.size();
  //if(trig) histo1D["trig_Tray1Muonhits"]->Fill(mNHitsR1);
  for(int i=0; i<mNHitsT2; i++){
     mHitsT2Vx[i]=hitsT2[i].x;//hitsT1[i].x;
     mHitsT2Vy[i]=hitsT2[i].y;
     mHitsT2Vz[i]=hitsT2[i].z;
     }
